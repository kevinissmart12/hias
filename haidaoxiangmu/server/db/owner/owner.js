const Ponds={
    ...
    add(option){
        const sql=`insert into owner (name,mobile,birthday,sex,imgUrl,address,county,town,village,engagedTime,educationLevel,farmerTitle,departmentId,discriminator,companyName,establishedTime,fixedAssets,businessContents,isBaseOfEmployee,numberOfEmployee,featuredProducts,creditLevel,comment,checkStatus,deleteStatus) values ('${option.name}','${option.mobile}','${option.birthday}','${option.sex}','${option.imgUrl}','${option.address}','${option.county}','${option.town}','${option.village}','${option.engagedTime}','${option.educationLevel}','${option.farmerTitle}','${option.departmentId}','${option.discriminator}','${option.companyName}','${option.establishedTime}','${option.fixedAssets}','${option.businessContents}','${option.isBaseOfEmployee}','${option.numberOfEmployee}','${option.featuredProducts}','${option.creditLevel}','${option.comment}','${option.checkStatus}','0')`
        return sql
    },
    ...
    //需要字段id
    delete(id){
        const sql=`update owner set deleteStatus='1' where id=('${id}')`

        return sql
    },
    updateDeleteStatus(id){
        const sql=`update owner set deleteStatus='0' where id=('${id}')`
